...
This workflow invokes three operations corresponding to each step: 1) a deposit transaction to a local account, 2) a communicator that invokes the withdrawal workflow for a target remote account, and 3) an undo transaction (only if the communicator fails).
...
We implement a [`updateAcctTransactionFunc`](./bank-backend/src/workflows/txnhistory.workflows.ts#L98) to perform a local bank deposit/withdrawal transaction, which updates (adds/subtracts) the user balance and appends it to the transaction history.
In the first step, the workflow invokes this transaction and specifies the transaction is a deposit:
```ts
...
Once the deposit transaction finishes, the workflow moves to the second step, which invokes a communicator function that sends an HTTP request to the remote backend server to invoke a `withdrawWorkflow` that withdraws the same amount of money from the target account:
```ts
...
First, the bank backend leverages the [`koa-jwt`](https://github.com/koajs/jwt) middleware for JWT verification:
```ts
...
The `koa-jwt` middleware sets a `state.user` object in the Koa context, which can be used by later middleware -- in our case, the Operon authentication middleware.
