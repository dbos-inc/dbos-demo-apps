...
1. You can easily perform reliable complex transactions through Operon workflows, while using a familiar ORM such as Prisma.
...
> We use the [`depositWorkflow`](./bank-backend/src/workflows/txnhistory.workflows.ts#L190) as an example, and its "twin" [`withdrawWorkflow`](./bank-backend/src/workflows/txnhistory.workflows.ts#L227) works similarly.
...
Any request that doesn't have a JWT token in its headers or the JWT token is invalid would be directly rejected with the status code set to `401`.

After the JWT token is authenticated, we proceed to authorization.
In bank, we specify two roles: "appAdmin" and "appUser".
...
Therefore, you can use the `@RequiredRole()` decorator to override the defaults.
```ts
@OperonTransaction()
...
We need a customized Operon authentication middleware to extract authenticated user and role information Koa context:
```ts
