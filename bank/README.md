...
The first step is a deposit transaction to a local account.
...
The workflow invokes this function and specifies the transaction is a deposit:
```ts
...
```
These two middleware functions are applied to each request from left to right: first, the `koaLogger` middleware logs the request and wraps all subsequent functions so it can correctly log the response; second, the `bankJwt` middleware verifies the JWT token.
Any request that doesn't have a valid JWT token in its headers is rejected with a `401` status code.
The `bankJwt` middleware also sets a `state.user` object in the Koa context, which can be used by later middleware -- in our case, the Operon authentication middleware.
