...
export class RespondUtilities {
  ...
  static async getAlertStatus() {
    const alerts = await knexds.client<AlertEmployee>('alert_employee').select().orderBy(['alert_id']);
  ...
  static async getMaxId() {
    const result = await knexds.client<AlertEmployee>('alert_employee').max('alert_id', { as: 'mid' }).first();
    if (result ) {
      return result.mid;
    }
    return -1;
